# IMU 标定程序 for 四轮车 (ROS2)

这是一个用于四轮车（仅平面运动）的 IMU 标定程序。它可以标定 IMU 相对于机器人中心 `base_link` 的安装位置偏差（`dx`, `dy`）和角度偏差（`yaw`、`roll`、`pitch`）。程序通过让机器人执行特定的运动（直线、旋转），结合 IMU、里程计和激光雷达数据，利用非线性优化求解安装参数。
